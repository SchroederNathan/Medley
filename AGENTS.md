...
- **REACT-NATIVE-2J**: `useUpdateChannel`'s `surfTo()` set the native `expo-updates` header override for the requested channel *before* confirming an update existed there, and only restored the previous override on the "no compatible update" path — the `catch` block (network error, or the channel having been deleted/mistyped, a hard 404) left the broken override in place. That override persists across app restarts, so every subsequent cold launch's automatic update check re-requested the dead channel and fell back to an "emergency launch" ("Expo Updates emergency launch: HTTP response error 404 ..."). Rule: any code that calls `Updates.setUpdateRequestHeadersOverride` (or otherwise mutates persistent native update state) must restore the previous value on *every* failure path, not just the expected/handled one — an unhandled exception is not a safe place to leave a persisted override dangling.
