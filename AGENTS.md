...
- **REACT-NATIVE-2G**: A second, smaller `<Svg><Filter><FeGaussianBlur stdDeviation="40">` behind `onboarding.tsx`'s bottom radial-gradient ellipse was left in place by the REACT-NATIVE-2D fix (explicitly called out as "out of scope" in that PR) and kept blocking the main thread on the onboarding screen's first paint for 2.8-3.6s. Rule: when fixing a main-thread-blocking SVG filter, grep the whole file (and sibling files) for every `<Filter>`/`FeGaussianBlur` occurrence before calling the fix done — a byte-for-byte-identical bug pattern can coexist with a near-identical sibling in the same file that a partial fix skips.
