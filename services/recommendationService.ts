...
function applyClientSideFilters(
  ...
  if (filters.includeGenresAny?.length) {
    ...
    results = results.filter((r) =>
      (r.unified_genres || []).some((g) => any.has(g))
    );
  }
  ...
  if (filters.excludeGenres?.length) {
    ...
    results = results.filter(
      (r) => !(r.unified_genres || []).some((g) => ex.has(g))
    );
  }
  return results;
}
...
async function fallbackByGenres(
  ...
  const filtered = (data as MediaWithUnified[]).filter(
    (r) => !ownedSet.has(r.id)
  );
...
export class RecommendationService {
  ...
  private static async getMediaRecommendationsFromMetadata(
    ...
    const recommendedIds: string[] = Array.isArray(
    ...
    const items: any[] = Array.isArray(providerBlock.items)
      ? providerBlock.items
      : [];
    ...
    const ordered = Array.from(unique.values()).sort((a, b) => {
      const aKey = isMovieOrTv
        ? a.external_ids?.tmdb_id
        : (a as any)?.external_ids?.igdb_id;
      const bKey = isMovieOrTv
        ? b.external_ids?.tmdb_id
        : (b as any)?.external_ids?.igdb_id;
  ...
  static async getByType(
    ...
    for (const src of (sources as any[]) || []) {
      const recs = await this.getMediaRecommendationsFromMetadata(
        src,
        40,
        targetType
      );
    ...
    const filteredFb = applyClientSideFilters(
      fb as MediaWithUnified[],
      filters
    );
  ...
  static async getAll(
    ...
    const merged = applyClientSideFilters(
      perType.flat() as MediaWithUnified[],
      filters
    );
  ...
  static async getSimilarToMedia(
    ...
    if (src) {
      ...
      if (recs.length > 0) {
        ...
        const filtered = applyClientSideFilters(
          filteredOwned as MediaWithUnified[],
          filters
        );
        return rankWithJitter(filtered, limit);
      }
  ...
  static async getFromFavorites(
    userId: string,
    filters?: RecommendationFilters
  ): Promise<Media[]> {
    const limit = filters?.limit ?? 20;
    const upstreamLimit = Math.min(Math.max(limit * 3, 20), 50);
    void userId;

    const results = await this.invokeEdgeFunction<Media[]>(
    ...
    const filtered = applyClientSideFilters(
      results as MediaWithUnified[],
      filters
    );

    return filtered.slice(0, limit) as Media[];
  }
}
